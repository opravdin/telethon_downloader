...
class TelethonDownloaderBot:
    ...
    def _add_handlers(self):
        try:
            ...
            self.bot.add_event_handler(self.handle_add_path_new_folder_name, events.NewMessage(incoming=True, func=lambda e: e.sender_id in self.AUTHORIZED_USER_IDS and e.message.text and not e.message.text.startswith('/') and (e.sender_id in self.commands_manager.active_add_extension_prompts or e.sender_id in self.commands_manager.active_add_group_prompts)))
            self.bot.add_event_handler(self.handle_add_extension_response, events.NewMessage(incoming=True, func=lambda e: e.sender_id in self.AUTHORIZED_USER_IDS and e.message.text and not e.message.text.startswith('/') and e.sender_id in self.commands_manager.active_add_extension_prompts))
            self.bot.add_event_handler(self.handle_add_group_response, events.NewMessage(incoming=True, func=lambda e: e.sender_id in self.AUTHORIZED_USER_IDS and e.message.text and not e.message.text.startswith('/') and e.sender_id in self.commands_manager.active_add_group_prompts))
    ...
    async def handle_add_group_response(self, event):
        await self.commands_manager.handle_group_id_input(event)
    ...
    async def handle_callback_query(self, event):
        try:
            ...
            if data == 'add_group_path':
                await self.commands_manager.start_add_group_path(event)
                return

            sender_id = event.sender_id
            prompt_info = self.commands_manager.active_add_extension_prompts.get(sender_id) or \
                          self.commands_manager.active_add_group_prompts.get(sender_id)

            if prompt_info and prompt_info.get('state') == 'waiting_for_path':
                parts = data.split('_')
                action = parts[0]
                ...
                if action == 'dir':
                    ...
                    prompt_info['current_dir'] = current_dir_from_state
                    
                    if nav_action == 'this':
                        path = current_dir_from_state
                        if sender_id in self.commands_manager.active_add_extension_prompts:
                            extension = prompt_info['extension']
                        ...
                        elif sender_id in self.commands_manager.active_add_group_prompts:
                            group_id = prompt_info['group_id']
                            self.config_manager.add_group_path(group_id, path)
                            await event.edit(f"Path for group '{group_id}' set to `{path}`.", buttons=None)
                            del self.commands_manager.active_add_group_prompts[sender_id]
                        return
                    ...
                    message_id = int(parts[1])
                    prompt_info['state'] = 'waiting_for_new_folder_name'
                    prompt_info['browser_message_id'] = message_id
    ...
    async def handle_add_path_new_folder_name(self, event):
        ...
        prompt_info = self.commands_manager.active_add_extension_prompts.get(sender_id) or \
                      self.commands_manager.active_add_group_prompts.get(sender_id)
        if not prompt_info or prompt_info.get('state') != 'waiting_for_new_folder_name':
            return
        ...
        try:
            ...
            prompt_info['state'] = 'waiting_for_path'
            prompt_info['current_dir'] = new_folder_path
